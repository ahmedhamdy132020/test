...
function medical_records_insert() {
	...
	if($data['image']) createThumbnail($data['image'], getThumbnailSpecs('medical_records', 'image', 'tv'));
	if($data['image']) createThumbnail($data['image'], getThumbnailSpecs('medical_records', 'image', 'dv'));
	$data['document'] = PrepareUploadedFile('document', 5120000,'txt|doc|docx|docm|odt|pdf|rtf', false, '');
	...
	if($_REQUEST['SelectedID']) {
		...
		if($row = db_fetch_assoc($res)) {
			...
			if(!$data['document']) $data['document'] = $row['document'];
		}
	}
	...
	insert('medical_records', backtick_keys_once($data), $error);
	if($error)
	...
	set_record_owner('medical_records', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) medical_records_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function medical_records_copy_children($destination_id, $source_id) {
	global $Translation;
...
function medical_records_update($selected_id) {
	...
	sql('update `medical_records` set       ' . ($data['image']!='' ? "`image`='{$data['image']}'" : ($_REQUEST['image_remove'] != 1 ? '`image`=`image`' : '`image`=NULL')) . ', ' . ($data['document']!='' ? "`document`='{$data['document']}'" : ($_REQUEST['document_remove'] != 1 ? '`document`=`document`' : '`document`=NULL')) . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!='') {
		echo $o['error'];
...
function medical_records_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$templateCode = str_replace('<%%UPLOADFILE(document)%%>', ($noUploads ? '' : "<div>{$Translation['upload image']}</div>" . '<i class="glyphicon glyphicon-remove text-danger clear-upload hidden"></i> <input type="file" name="document" id="document" data-filetypes="txt|doc|docx|docm|odt|pdf|rtf" data-maxsize="5120000">'), $templateCode);
	...
	if($selected_id) {
		...
		if($jdata === false) $jdata = get_defaults('medical_records');
		$rdata = $row;
	}
	$templateCode .= loadView('medical_records-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
