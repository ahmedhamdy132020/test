	...
	*/
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(
			...
			'medical_img' => "`medical_img` LEFT JOIN `patients` as patients1 ON `patients1`.`id`=`medical_img`.`patient` ",
			'medical_docs' => "`medical_docs` LEFT JOIN `patients` as patients1 ON `patients1`.`id`=`medical_docs`.`patient` ",
		);
	...
	function htmlUserBar() {
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> online clinic management system</a>
			</div>
	...
	function NavMenus($options = array()) {
		...
		if(is_array($arrTables)) {
			foreach($arrTables as $tn => $tc) {
				...
				$tChkHL = array_search($tn, array('medical_img','medical_docs'));
