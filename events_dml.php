...
function events_insert() {
	...
	$data['name_patient'] = $_REQUEST['name_patient'];
	...
	$data['prescription'] = $_REQUEST['prescription'];
		if($data['prescription'] == empty_lookup_value) { $data['prescription'] = ''; }
	$data['diagnosis'] = $_REQUEST['diagnosis'];
	...
	insert('events', backtick_keys_once($data), $error);
	if($error)
	...
	if(!empty($_REQUEST['SelectedID'])) events_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function events_copy_children($destination_id, $source_id) {
	global $Translation;
...
function events_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	<script>
		...
		AppGini.current_name_patient__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['name_patient'] : $filterer_name_patient); ?>"};
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#name_patient-container__RAND__").select2({
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { s: term, p: page, t: 'events', f: 'name_patient' }; },
	...
	$lookup_fields = array('name_patient' => array('patients', 'Patient Name'), );
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(name_patient)%%>', safe_html($urow['name_patient']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(name_patient)%%>', html_attr($row['name_patient']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(time)%%>', time12('12:00'), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(time)%%>', urlencode(time12('12:00')), $templateCode);
	...
	$templateCode .= loadView('events-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
