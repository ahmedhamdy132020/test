...
// Data functions (insert, update, delete, form) for table medical_img

// This script and data application were generated by AppGini 5.82
// Download AppGini for free from https://bigprof.com/appgini/download/

function medical_img_insert() {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('medical_img');
	...
	if($_REQUEST['SelectedID']) {
		$res = sql("select * from medical_img where id='" . makeSafe($_REQUEST['SelectedID']) . "'", $eo);
		if($row = db_fetch_assoc($res)) {
			if(!$data['image']) $data['image'] = $row['image'];
		}
	}

	// hook: medical_img_before_insert
	if(function_exists('medical_img_before_insert')) {
		$args = array();
		if(!medical_img_before_insert($data, getMemberInfo(), $args)) { return false; }
	}

	$error = '';
	// set empty fields to NULL
	$data = array_map(function($v) { return ($v === '' ? NULL : $v); }, $data);
	insert('medical_img', backtick_keys_once($data), $error);
	if($error)
	...
	if($_REQUEST['filterer_patient']) {
		sql("update `medical_img` set `patient`='" . makeSafe($_REQUEST['filterer_patient']) . "' where `id`='" . makeSafe($recID, false) . "'", $eo);
	}

	// hook: medical_img_after_insert
	if(function_exists('medical_img_after_insert')) {
		$res = sql("select * from `medical_img` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!medical_img_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('medical_img', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) medical_img_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function medical_img_copy_children($destination_id, $source_id) {
	global $Translation;
...
function medical_img_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false) {
	...
	$arrPerm=getTablePermissions('medical_img');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='medical_img' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='medical_img' and pkValue='$selected_id'");
	...
	// hook: medical_img_before_delete
	if(function_exists('medical_img_before_delete')) {
		$args=array();
		if(!medical_img_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	if(function_exists('medical_img_after_delete')) {
		$args=array();
		medical_img_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='medical_img' and pkValue='$selected_id'", $eo);
}

function medical_img_update($selected_id) {
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('medical_img');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='medical_img' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='medical_img' and pkValue='".makeSafe($selected_id)."'");
	...
	if(function_exists('medical_img_before_update')) {
		$args = array();
		if(!medical_img_before_update($data, getMemberInfo(), $args)) { return false; }
	}
	...
	// hook: medical_img_after_update
	if(function_exists('medical_img_after_update')) {
		$res = sql("SELECT * FROM `medical_img` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!medical_img_after_update($data, getMemberInfo(), $args)) { return; }
	}

	// mm: update ownership data
	sql("update `membership_userrecords` set `dateUpdated`='" . time() . "' where `tableName`='medical_img' and `pkValue`='" . makeSafe($selected_id) . "'", $eo);

}

function medical_img_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$res = sql("SELECT * FROM `medical_img` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'medical_img_view.php', false);
		}
			...
			$j("#patient-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_patient__RAND__.value, t: 'medical_img', f: 'patient' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { /* */ return { s: term, p: page, t: 'medical_img', f: 'patient' }; },
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/medical_img_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/medical_img_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}

	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Medical img details', $templateCode);
	$templateCode = str_replace('<%%RND1%%>', $rnd1, $templateCode);
	$templateCode = str_replace('<%%EMBEDDED%%>', ($_REQUEST['Embedded'] ? 'Embedded=1' : ''), $templateCode);
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return medical_img_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return medical_img_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return medical_img_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$templateCode = str_replace('<%%UPLOADFILE(image)%%>', ($noUploads ? '' : "<div>{$Translation['upload image']}</div>" . '<i class="glyphicon glyphicon-remove text-danger clear-upload hidden"></i> <input type="file" name="image" id="image" data-filetypes="jpg|jpeg|gif|png" data-maxsize="1024000">'), $templateCode);
	...
	$rdata = $jdata = get_defaults('medical_img');
	if($selected_id) {
		$jdata = get_joined_record('medical_img', $selected_id);
		if($jdata === false) $jdata = get_defaults('medical_img');
		$rdata = $row;
	}
	$templateCode .= loadView('medical_img-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: medical_img_dv
	if(function_exists('medical_img_dv')) {
		$args=array();
		medical_img_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
