...
function patients_insert() {
	...
	$data['sexual_orientation'] = $_REQUEST['sexual_orientation'];
	...
	$data['surgical_history'] = br2nl($_REQUEST['surgical_history']);
	$data['obstetric_history'] = br2nl($_REQUEST['obstetric_history']);
	$data['genetic_diseases'] = br2nl($_REQUEST['genetic_diseases']);
	$data['contact_person'] = $_REQUEST['contact_person'];
		if($data['contact_person'] == empty_lookup_value) { $data['contact_person'] = ''; }
	$data['other_details'] = br2nl($_REQUEST['other_details']);
	...
	if($data['sexual_orientation']== '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">" . $Translation['error:'] . " 'sexual_orientation': " . $Translation['field not null'] . '<br><br>';
		echo '<a href="" onclick="history.go(-1); return false;">'.$Translation['< back'].'</a></div>';
		exit;
	}
	if($data['birth_date']== '') {
	...
	insert('patients', backtick_keys_once($data), $error);
	if($error)
	...
	set_record_owner('patients', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) patients_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function patients_copy_children($destination_id, $source_id) {
	global $Translation;
...
function patients_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false) {
	...
	// child table: medical_img
	$res = sql("select `id` from `patients` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `medical_img` where `patient`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "medical_img", $RetMsg);
		return $RetMsg;
	}elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["confirm delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	// child table: medical_docs
	$res = sql("select `id` from `patients` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `medical_docs` where `patient`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "medical_docs", $RetMsg);
		return $RetMsg;
	}elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["confirm delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function patients_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(sexual_orientation)%%>', safe_html($urow['sexual_orientation']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(sexual_orientation)%%>', html_attr($row['sexual_orientation']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(home_phone)%%>', safe_html($urow['home_phone']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(home_phone)%%>', html_attr($row['home_phone']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(tobacco_usage)%%>', safe_html($urow['tobacco_usage']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(tobacco_usage)%%>', html_attr($row['tobacco_usage']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(tobacco_usage)%%>', urlencode($urow['tobacco_usage']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(alcohol_intake)%%>', safe_html($urow['alcohol_intake']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(alcohol_intake)%%>', html_attr($row['alcohol_intake']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(alcohol_intake)%%>', urlencode($urow['alcohol_intake']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(history)%%>', safe_html($urow['history']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(history)%%>', html_attr($row['history']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(history)%%>', urlencode($urow['history']), $templateCode);
		if($dvprint || (!$AllowUpdate && !$AllowInsert)) {
			$templateCode = str_replace('<%%VALUE(surgical_history)%%>', safe_html($urow['surgical_history']), $templateCode);
		}else{
		...
		if($dvprint || (!$AllowUpdate && !$AllowInsert)) {
			$templateCode = str_replace('<%%VALUE(obstetric_history)%%>', safe_html($urow['obstetric_history']), $templateCode);
		}else{
		...
		if($dvprint || (!$AllowUpdate && !$AllowInsert)) {
			$templateCode = str_replace('<%%VALUE(genetic_diseases)%%>', safe_html($urow['genetic_diseases']), $templateCode);
		}else{
	...
	if($selected_id) {
		...
		if($jdata === false) $jdata = get_defaults('patients');
		$rdata = $row;
	}
	$templateCode .= loadView('patients-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
