		...
		$pcConfig = array(
			...
			'events' => array(
				'name_patient' => array(
					...
					'tab-label' => 'Appointments <span class="hidden child-label-events child-field-caption">(Patient Name)</span>',
					'auto-close' => false,
					'table-icon' => 'resources/table_icons/clock_.png',
					...
					'sortable-fields' => array(0 => '`events`.`id`', 1 => 2, 2 => '`events`.`date`', 3 => 4, 4 => 5, 5 => '`events`.`time`', 6 => 7, 7 => 8, 8 => 9),
					'records-per-page' => 10,
					'default-sort-by' => 0,
			...
			'medical_img' => array(
				'patient' => array(
					...
					'child-primary-key' => 'id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Medical img <span class="hidden child-label-medical_img child-field-caption">(Patient)</span>',
					'auto-close' => false,
					'table-icon' => 'resources/table_icons/action_log.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Patient', 2 => 'Image', 3 => 'Description'),
					'display-field-names' => array(1 => 'patient', 2 => 'image', 3 => 'description'),
					'sortable-fields' => array(0 => '`medical_img`.`id`', 1 => 2, 2 => 3, 3 => 4),
					'records-per-page' => 10,
					'default-sort-by' => 0,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-medical_img',
					'template-printable' => 'children-medical_img-printable',
					'query' => "SELECT `medical_img`.`id` as 'id', IF(    CHAR_LENGTH(`patients1`.`last_name`) || CHAR_LENGTH(`patients1`.`first_name`), CONCAT_WS('',   `patients1`.`last_name`, ', ', `patients1`.`first_name`), '') as 'patient', `medical_img`.`image` as 'image', `medical_img`.`description` as 'description' FROM `medical_img` LEFT JOIN `patients` as patients1 ON `patients1`.`id`=`medical_img`.`patient` "
				),
			),
			'medical_docs' => array(
				'patient' => array(
					...
					'child-primary-key' => 'id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Medical docs <span class="hidden child-label-medical_docs child-field-caption">(Patient)</span>',
					'auto-close' => false,
					'table-icon' => 'resources/table_icons/active_sessions.png',
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Patient', 2 => 'Doc', 3 => 'Description'),
					'display-field-names' => array(1 => 'patient', 2 => 'doc', 3 => 'description'),
					'sortable-fields' => array(0 => '`medical_docs`.`id`', 1 => 2, 2 => 3, 3 => 4),
					'records-per-page' => 10,
					'default-sort-by' => 0,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-medical_docs',
					'template-printable' => 'children-medical_docs-printable',
					'query' => "SELECT `medical_docs`.`id` as 'id', IF(    CHAR_LENGTH(`patients1`.`last_name`) || CHAR_LENGTH(`patients1`.`first_name`), CONCAT_WS('',   `patients1`.`last_name`, ', ', `patients1`.`first_name`), '') as 'patient', `medical_docs`.`doc` as 'doc', `medical_docs`.`description` as 'description' FROM `medical_docs` LEFT JOIN `patients` as patients1 ON `patients1`.`id`=`medical_docs`.`patient` "
				),
			),
		);
