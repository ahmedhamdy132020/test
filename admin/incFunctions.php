<?php
	...
	*/
	...
	function getThumbnailSpecs($tableName, $fieldName, $view) {
		...
		elseif($tableName=='patients' && $fieldName=='image' && $view=='dv')
			return array('width'=>250, 'height'=>250, 'identifier'=>'_dv');
		elseif($tableName=='medical_img' && $fieldName=='image' && $view=='tv')
			return array('width'=>50, 'height'=>50, 'identifier'=>'_tv');
		elseif($tableName=='medical_img' && $fieldName=='image' && $view=='dv')
			return array('width'=>250, 'height'=>250, 'identifier'=>'_dv');
		return FALSE;
	}
	########################################################################
	...
	function initSession() {
		...
		@session_name('online_clinic_management_system');
		session_start();
	}
	########################################################################
	function jwt_key() {
...
########################################################################
	...
	*/
	...
	function calculated_fields() {
		...
		return array(
			'patients' => array(
				'age' => 'SELECT TRUNCATE(DATEDIFF(DATE(NOW()),`patients`.`birth_date`)/365,0) FROM `patients` 
